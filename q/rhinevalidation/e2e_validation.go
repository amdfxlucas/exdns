...
func RhineRRSigCheckROA(in *dns.Msg, roa *ROA) {
	if roa != nil {
		key := roa.dnskey
		RhineRRSigCheck(in, key)
	}
}
		...
		if rr.Header().Rrtype == dns.TypeRRSIG {
			...
			if err := rr.(*dns.RRSIG).Verify(key, rrset); err != nil {
				fmt.Printf("[RHINE] ;- Bogus signature, %s does not validate (DNSKEY %s/%d) [%s] %s\n",
					util.ShortSig(rr.(*dns.RRSIG)), key.Header().Name, key.KeyTag(), err.Error(), expired)
			} else {
				fmt.Printf("[RHINE] ;+ Secure signature, %s validates (DNSKEY %s/%d) %s\n", util.ShortSig(rr.(*dns.RRSIG)), key.Header().Name, key.KeyTag(), expired)
			}
		}
	}
}
